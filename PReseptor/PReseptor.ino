...
void loop() {
  ...
  if (flag == 4) {                    //con flag = 4 reconstruimos el valor guardado en binario a decimal y lo guardamos en el array patron para recrear el color con rgb
    if(ER[0] == 1){ patron[0]=+128;}
     if(ER[1] == 1){ patron[0]=+64;}
     if(ER[2] == 1){ patron[0]=+32;}
     if(ER[3] == 1){ patron[0]=+16;}
     if(ER[4] == 1){ patron[0]=+8;}
     if(ER[5] == 1){ patron[0]=+4;}
     if(ER[6] == 1){ patron[0]=+2;}
     if(ER[7] == 1){ patron[0]=+1;}

     if(EG[0] == 1){ patron[1]=+128;}
     if(EG[1] == 1){ patron[1]=+64;}
     if(EG[2] == 1){ patron[1]=+32;}
     if(EG[3] == 1){ patron[1]=+16;}
     if(EG[4] == 1){ patron[1]=+8;}
     if(EG[5] == 1){ patron[1]=+4;}
     if(EG[6] == 1){ patron[1]=+2;}
     if(EG[7] == 1){ patron[1]=+1;}

     if(EB[0] == 1){ patron[2]=+128;}
     if(EB[1] == 1){ patron[2]=+64;}
     if(EB[2] == 1){ patron[2]=+32;}
     if(EB[3] == 1){ patron[2]=+16;}
     if(EB[4] == 1){ patron[2]=+8;}
     if(EB[5] == 1){ patron[2]=+4;}
     if(EB[6] == 1){ patron[2]=+2;}
     if(EB[7] == 1){ patron[2]=+1;}
